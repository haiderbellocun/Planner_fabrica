...
const temaLeaderMiddleware = projectLeaderOfResourceMiddleware(async (req) => {
    const result = await query(`SELECT a.project_id FROM public.temas t JOIN public.asignaturas a ON a.id = t.asignatura_id WHERE t.id = $1`, [req.params.id ?? req.params.temaId]);
    return result.rows[0]?.project_id ?? null;
});
