...
import { login, register, getCurrentUser, logout, googleCallback, } from '../controllers/authController.js';
import { authMiddleware } from '../middleware/auth.js';
import { env } from '../config/env.js';
const router = express.Router();
function getFrontendOrigin() {
    const raw = env.FRONTEND_URL || env.CORS_ORIGIN.split(',')[0]?.trim() || 'http://localhost:8080';
    return raw.replace(/\/$/, '');
}
    ...
    router.get('/google/callback', (req, res, next) => {
        ...
        passport.authenticate('google', { session: false }, (err, user) => {
            if (err || !user) {
                ...
                const errorCode = msg === 'USER_NOT_FOUND'
                    ? 'not_found'
