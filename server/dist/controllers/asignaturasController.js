...
export const listAsignaturas = async (req, res) => {
    try {
       ...
       GROUP BY a.id
       ORDER BY a.display_order ASC, a.created_at ASC`, [projectId]);
 ...
 */
export const listAsignaturasByPrograma = async (req, res) => {
    try {
       ...
       GROUP BY a.id
       ORDER BY a.display_order ASC, a.created_at ASC`, [programaId]);
        res.json(result.rows);
    }
    catch (error) {
...
export const getAsignatura = async (req, res) => {
    try {
        const { id } = req.params;
        const asignaturaResult = await query('SELECT * FROM public.asignaturas WHERE id = $1', [id]);
        ...
        const materialesResult = await query(`SELECT mr.*, mt.name as material_name, mt.icon as material_icon
       FROM public.materiales_requeridos mr
       JOIN public.material_types mt ON mt.id = mr.material_type_id
       WHERE mr.asignatura_id = $1
       ORDER BY mt.display_order ASC`, [id]);
        res.json({
            ...asignatura,
...
export const createAsignatura = async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO public.asignaturas (project_id, name, code, description, display_order)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [projectId, name, code || null, description || null, display_order || 0]);
...
export const updateAsignatura = async (req, res) => {
    try {
        ...
        values.push(id);
        const result = await query(`UPDATE public.asignaturas
       ...
       RETURNING *`, values);
        ...
        if (completado === true) {
            await query(`UPDATE public.temas SET completado = true, updated_at = NOW() WHERE asignatura_id = $1`, [id]);
            await query(`UPDATE public.materiales_requeridos mr
         SET completado = true, updated_at = NOW()
         FROM public.temas t
         WHERE mr.tema_id = t.id AND t.asignatura_id = $1`, [id]);
        }
...
export const deleteAsignatura = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await query('DELETE FROM public.asignaturas WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Asignatura not found' });
        }
 ...
 */
export const getTemasWithMateriales = async (req, res) => {
    try {
        const { id } = req.params;
       ...
       ORDER BY t.display_order ASC, t.created_at ASC`, [id]);
        // For each tema, get its materials
        const temasWithMateriales = await Promise.all(temasResult.rows.map(async (tema) => {
            const materialesResult = await query(`SELECT mr.*, mt.name, mt.icon, mt.description
           FROM public.materiales_requeridos mr
           JOIN public.material_types mt ON mt.id = mr.material_type_id
           WHERE mr.tema_id = $1
