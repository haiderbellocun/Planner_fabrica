 ...
 * - user: 403.
 */
export const getLeadersFocus = async (req, res) => {
    ...
    try {
        if (userRole === 'admin') {
            ...
            const validProfileId = typeof profileId === 'string' && profileId.trim() !== '' ? profileId.trim() : undefined;
            if (!validProfileId) {
            ...
            const scopeResult = await query('SELECT cargo FROM public.leader_cargo_scope WHERE leader_profile_id = $1', [validProfileId]);
            cargos = (scopeResult?.rows ?? []).map((r) => r.cargo).filter(Boolean);
            if (cargos.length === 0) {
                // Sin scope de cargo: mostrar todas las tareas de proyectos donde es líder
                const result = await query(`${TASKS_SELECT_FOR_LEADER_PROJECTS} ORDER BY t.due_date ASC`, [validProfileId]);
                return res.json(mapRowsToTasks(result.rows ?? []));
            }
        }
        const sql = cargos && cargos.length > 0
