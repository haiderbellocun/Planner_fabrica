...
export const createTaskComment = async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO public.task_comments (task_id, user_id, comment)
       VALUES ($1, $2, $3)
       RETURNING *`, [taskId, userId, comment.trim()]);
        const newComment = result.rows[0];
        // Get user info
        const userResult = await query(`SELECT id, full_name, avatar_url, email
...
export const deleteTaskComment = async (req, res) => {
    try {
        ...
        const commentResult = await query('SELECT user_id FROM public.task_comments WHERE id = $1', [commentId]);
