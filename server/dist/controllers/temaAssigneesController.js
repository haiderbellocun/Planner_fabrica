 ...
 */
export const updateTemaAssignees = async (req, res) => {
    try {
        ...
        const taskResult = await query('SELECT project_id FROM public.tasks WHERE id = $1', [taskId]);
 ...
 */
export const getTemaAssignees = async (req, res) => {
    try {
        const { taskId } = req.params;
        const result = await query(`SELECT
        tta.id, tta.task_id, tta.tema_id, tta.assignee_id,
