...
export const listProfiles = async (req, res) => {
    try {
        const result = await query(`SELECT p.id, p.full_name, p.avatar_url, p.email, p.cargo, p.created_at
...
export const updateCapacity = async (req, res) => {
    try {
        ...
        const result = await query('UPDATE public.profiles SET weekly_hours_capacity = $1 WHERE id = $2 RETURNING id, weekly_hours_capacity', [value, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Profile not found' });
        }
        res.json(result.rows[0]);
