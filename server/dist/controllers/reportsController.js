...
import { ASSIGNED_WORK_CTE, TASK_COMPLETION_CTE, TASK_ASSIGNEE_COUNT_CTE, EFFORT_HOURS_CTE, REPORT_TZ, WORK_SCHEDULE, addBusinessDays, riskBand, assertEnum, validUuidOrNull, validDateOrNull, } from './reportsMetrics.js';
/**
 ...
 */
export const getMaterialProduction = async (req, res) => {
    try {
 ...
 */
export const getTimeDistribution = async (req, res) => {
    try {
    ...
    `);
        const distribution = result.rows.map(r => ({
            ...
            display_order: r.display_order,
            count: r.count,
            durations_hours: [], // kept for type compatibility; empty — stats are in .stats
            stats: {
...
export const getTeamCapacity = async (req, res) => {
    ...
    `);
        const members = result.rows.map(r => {
            ...
            const memberWeekly = r.weekly_hours_capacity != null && !Number.isNaN(Number(r.weekly_hours_capacity))
            ...
            const estimatedWorkDays = pendingHoras > 0 && memberDaily > 0 ? Math.round((pendingHoras / memberDaily) * 10) / 10 : 0;
 ...
 */
export const getTimeByPhase = async (req, res) => {
    try {
        const result = await query(`
      SELECT
        p.id                                                     AS profile_id,
 ...
 */
export const getTasksDetail = async (req, res) => {
    try {
        const result = await query(`
      WITH phase_times AS (
        SELECT
 ...
 */
export const getOntimeByEquipo = async (req, res) => {
    try {
 ...
 */
export const getPersonMetrics = async (req, res) => {
    try {
...
export const getCapacityForecast = async (req, res) => {
    ...
    `, [projectId]);
        ...
        for (const m of members)
            riskCounts[m.current.risk_level]++;
        res.json({
            schedule: {
                mon_thu_hours: WORK_SCHEDULE.MON_THU_HOURS,
                friday_hours: WORK_SCHEDULE.FRIDAY_HOURS,
...
export const getThroughput = async (req, res) => {
    try {
        const bucket = assertEnum(req.query.bucket, ['day', 'week', 'month'], 'week');
        const groupBy = assertEnum(req.query.group_by, ['person', 'cargo'], 'person');
        const projectId = validUuidOrNull(req.query.project_id);
        const dateFrom = validDateOrNull(req.query.date_from);
        const dateTo = validDateOrNull(req.query.date_to);
 ...
 */
export const getProductionByPerson = async (req, res) => {
    try {
        const projectId = validUuidOrNull(req.query.project_id);
        const dateFrom = validDateOrNull(req.query.date_from);
        const dateTo = validDateOrNull(req.query.date_to);
    ...
    `, [projectId, dateFrom, dateTo]);
        ...
        const totalsByPersonMap = new Map();
