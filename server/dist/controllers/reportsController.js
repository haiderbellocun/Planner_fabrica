...
export const getTimeDistribution = async (req, res) => {
    ...
    `);
        const distribution = result.rows.map(r => {
            ...
            const hoursArray = durations.map((d) => d / 3600);
            // Compute stats
            ...
            const mean = len > 0 ? sorted.reduce((a, b) => a + b, 0) / len : 0;
...
export const getTeamCapacity = async (req, res) => {
    ...
    `);
        const members = result.rows.map(r => {
            ...
            const utilizationPct = memberWeekly > 0 ? Math.round((pendingHoras / memberWeekly) * 100) : 0;
