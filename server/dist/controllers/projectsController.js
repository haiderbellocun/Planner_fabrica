...
export const createProject = async (req, res) => {
    try {
        ...
        try {
         ...
         VALUES ($1, $2, 'leader', true, true, true, true, $2)`, [project.id, profileId]);
            ...
            const leadersResult = await query(`SELECT p.id FROM public.profiles p
         JOIN public.users u ON u.id = p.user_id
         WHERE u.email = ANY($1)`, [defaultLeaderEmails]);
            for (const leader of leadersResult.rows) {
                // Skip if the leader is the creator (already added above)
                if (leader.id !== profileId) {
                    await query(`INSERT INTO public.project_members (project_id, user_id, role, can_view, can_create, can_edit, can_assign, invited_by)
             VALUES ($1, $2, 'leader', true, true, true, true, $3)`, [project.id, leader.id, profileId]);
                }
            }
            ...
            if (asignaturas && Array.isArray(asignaturas) && asignaturas.length > 0) {
                for (let i = 0; i < asignaturas.length; i++) {
                    ...
                    // Insert asignatura
                    const asignaturaResult = await query(`INSERT INTO public.asignaturas (project_id, name, code, description, display_order)
             VALUES ($1, $2, $3, $4, $5)
             RETURNING *`, [project.id, asignatura.name, asignatura.code || null, asignatura.description || null, i]);
                    ...
                    if (asignatura.materiales && Array.isArray(asignatura.materiales)) {
                        for (const material of asignatura.materiales) {
                            await query(`INSERT INTO public.materiales_requeridos (asignatura_id, material_type_id, cantidad, descripcion)
                 VALUES ($1, $2, $3, $4)`, [asignaturaId, material.material_type_id, material.cantidad || 1, material.descripcion || null]);
                        }
                    }
                }
            }
            ...
            query(`SELECT u.email, p.full_name
         FROM public.user_roles ur
         JOIN public.profiles p ON p.id = ur.user_id
         JOIN public.users u ON u.id = p.user_id
         WHERE ur.role = 'project_leader'
           AND u.is_active = true`, []).then(async (leadersResult) => {
...
export const updateProject = async (req, res) => {
    try {
        ...
        values.push(id);
        const result = await query(`UPDATE public.projects
...
export const deleteProject = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await query('DELETE FROM public.projects WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Project not found' });
        }
...
export const addMember = async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO public.project_members (project_id, user_id, role, can_view, can_create, can_edit, can_assign, invited_by)
       VALUES ($1, $2, $3, true, $4, $5, $6, $7)
       RETURNING *`, [projectId, user_id, role || 'member', can_create || false, can_edit || false, can_assign || false, invitedBy]);
