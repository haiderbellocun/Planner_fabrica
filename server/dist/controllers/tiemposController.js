 ...
 */
export const getTiemposEstimados = async (req, res) => {
    try {
 ...
 */
export const calcularTiempo = async (req, res) => {
    try {
       ...
       ORDER BY te.cantidad_valor`, [material_type_id, cargo]);
        ...
        if (cantidad_valor) {
            const val = parseFloat(cantidad_valor);
            ...
            const sorted = result.rows.sort((a, b) => a.cantidad_valor - b.cantidad_valor);
            let lower = null;
            let upper = null;
            for (const row of sorted) {
                if (parseFloat(row.cantidad_valor) <= val)
                    lower = row;
                if (parseFloat(row.cantidad_valor) >= val && !upper)
                    upper = row;
            }
 ...
 */
export const calcularTiempoTarea = async (req, res) => {
    try {
        const { taskId } = req.params;
        // Get task with material and assignee info
        const taskResult = await query(`SELECT t.id, t.title, t.assignee_id,
       ...
       WHERE t.id = $1`, [taskId]);
        ...
        if (task.material_type_id && task.assignee_cargo) {
            const tiemposResult = await query(`SELECT * FROM public.tiempos_estimados
         WHERE material_type_id = $1 AND LOWER(cargo) = LOWER($2)
         ORDER BY cantidad_valor`, [task.material_type_id, task.assignee_cargo]);
            taskTiempos = tiemposResult.rows;
        }
        // Always get material assignees and their estimations
        const materialAssigneesResult = await query(`SELECT tma.material_id, tma.assignee_id,
              p.full_name, p.cargo,
       ...
       WHERE tma.task_id = $1`, [taskId]);
        ...
        for (const ma of materialAssigneesResult.rows) {
            ...
            let tiempoResult = await query(`SELECT * FROM public.tiempos_estimados
         WHERE material_type_id = $1 AND LOWER(cargo) = LOWER($2)
         ...
         ORDER BY producto, cantidad_valor`, [ma.material_type_id, ma.cargo || '']);
            // Fallback: if no cargo-specific match, show ALL tiempos for this material type
            if (tiempoResult.rows.length === 0) {
                tiempoResult = await query(`SELECT * FROM public.tiempos_estimados
           WHERE material_type_id = $1
           AND LOWER(producto) NOT LIKE 'plantilla%'
           AND LOWER(producto) NOT LIKE 'guion%'
           AND LOWER(producto) NOT LIKE 'guión%'
           AND LOWER(producto) NOT LIKE 'post %'
           ORDER BY producto, cantidad_valor`, [ma.material_type_id]);
            }
...
export const getProductos = async (_req, res) => {
    try {
        const result = await query(`SELECT DISTINCT producto FROM public.tiempos_estimados ORDER BY producto`);
 ...
 */
export const getCargos = async (_req, res) => {
    try {
        const result = await query(`SELECT DISTINCT cargo FROM public.tiempos_estimados ORDER BY cargo`);
