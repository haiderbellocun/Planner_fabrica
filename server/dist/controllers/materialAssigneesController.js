 ...
 */
export const updateMaterialAssignees = async (req, res) => {
    try {
        ...
        const taskResult = await query(`SELECT t.*, ts.name as status_name
       FROM public.tasks t
       ...
       WHERE t.id = $1`, [taskId]);
        ...
        try {
            ...
            if (parentTask.parent_task_id) {
                const parentOfParent = await query('SELECT parent_task_id FROM public.tasks WHERE id = $1', [parentTask.parent_task_id]);
            ...
            if (shouldCreateUserTasks && defaultStatusId && newAssignments.length > 0) {
                for (const assignment of newAssignments) {
                    ...
                    const existingTask = await query(`SELECT id FROM public.tasks
             WHERE parent_task_id = $1
             AND material_requerido_id = $2
             AND assignee_id = $3`, [taskId, assignment.material_id, assignment.assignee_id]);
                    if (existingTask.rows.length > 0) {
                        // Task already exists for this assignment, skip
                        continue;
                    }
                    // Only create tasks for regular members — skip project leaders
                    const leaderCheck = await query('SELECT public.is_project_leader($1::UUID, $2::UUID) as is_leader', [projectId, assignment.assignee_id]);
                    if (leaderCheck.rows[0]?.is_leader) {
                        continue;
                    }
                    // Get material type name for the task title
                    const materialInfo = await query(`SELECT mt.name as type_name, mr.descripcion
             FROM public.materiales_requeridos mr
             JOIN public.material_types mt ON mt.id = mr.material_type_id
             WHERE mr.id = $1`, [assignment.material_id]);
 ...
 */
export const getMaterialAssignees = async (req, res) => {
    try {
        const { taskId } = req.params;
        const result = await query(`SELECT
        tma.id, tma.task_id, tma.material_id, tma.assignee_id, tma.horas_estimadas,
