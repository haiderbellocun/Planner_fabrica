 ...
 */
export const listTemas = async (req, res) => {
    try {
       ...
       GROUP BY t.id
       ORDER BY t.display_order ASC, t.created_at ASC`, [asignaturaId]);
...
export const getTema = async (req, res) => {
    try {
        const { id } = req.params;
        const temaResult = await query('SELECT * FROM public.temas WHERE id = $1', [id]);
...
export const createTema = async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO public.temas (asignatura_id, title, description, display_order)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [asignaturaId, title.trim(), description || null, display_order || 0]);
...
export const updateTema = async (req, res) => {
    try {
        ...
        values.push(id);
        const result = await query(`UPDATE public.temas
...
export const deleteTema = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await query('DELETE FROM public.temas WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Tema not found' });
        }
