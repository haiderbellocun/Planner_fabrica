 ...
 */
export const listMaterialTypes = async (req: AuthRequest, res: Response) => {
  try {
    const result = await query(
      'SELECT * FROM public.material_types ORDER BY display_order ASC'
    );
 ...
 */
export const listMaterialesForAsignatura = async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
export const createMaterialRequerido = async (req: AuthRequest, res: Response) => {
  try {
    const { asignaturaId } = req.params;
    ...
    const result = await query(
      `INSERT INTO public.materiales_requeridos (asignatura_id, material_type_id, cantidad, descripcion)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [asignaturaId, material_type_id, cantidad || 1, descripcion || null]
    );
...
export const updateMaterialRequerido = async (req: AuthRequest, res: Response) => {
  try {
    ...
    values.push(id);
    const result = await query(
      `UPDATE public.materiales_requeridos
 ...
 */
export const deleteMaterialRequerido = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;

    const result = await query(
      'DELETE FROM public.materiales_requeridos WHERE id = $1 RETURNING id',
      [id]
    );
 ...
 */
export const listMaterialesForTema = async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
export const createMaterialRequeridoForTema = async (req: AuthRequest, res: Response) => {
  try {
    const { temaId } = req.params;
    const { material_type_id, cantidad, descripcion } = req.body;

    const result = await query(
      `INSERT INTO public.materiales_requeridos (tema_id, material_type_id, cantidad, descripcion)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [temaId, material_type_id, cantidad || 1, descripcion || null]
    );
