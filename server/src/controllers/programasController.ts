 ...
 */
export const listProgramas = async (req: AuthRequest, res: Response) => {
  try {
    const { projectId } = req.params;

    // Get all programas for the project
    const programasResult = await query(
      `SELECT p.*
 ...
 */
export const getPrograma = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;

    const programaResult = await query(
      'SELECT * FROM public.programas WHERE id = $1',
      [id]
    );
 ...
 * Create new programa
 */
export const createPrograma = async (req: AuthRequest, res: Response) => {
  try {
    const { projectId } = req.params;
    ...
    const result = await query(
      `INSERT INTO public.programas (project_id, name, code, description, display_order, tipo_programa)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [projectId, name.trim(), code || null, description || null, display_order || 0, tipo_programa || null]
    );
 ...
 * Update programa
 */
export const updatePrograma = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { name, code, description, display_order, tipo_programa } = req.body;
 ...
 */
export const deletePrograma = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;

    const result = await query(
      'DELETE FROM public.programas WHERE id = $1 RETURNING id',
      [id]
    );
